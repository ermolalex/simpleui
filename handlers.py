...
def on_start_active_cv(hashMap,_files=None,_data=None):
    ...
    info_list = []
    for link in results:
        job = {"object":str(link['barcode']),"info":str(link['name'])+" </n> РћСЃС‚Р°С‚РѕРє: <big>"+str(link['qty'])+"</big>"}
...
def settings_on_input(hashMap,_files=None,_data=None):
    if hashMap.get("listener")=="lang":
        ...
        if hashMap.get("lang")=="Р СѓСЃСЃРєРёР№":
            hashMap.put("setLocale","ru")   
        elif hashMap.get("lang")=="РђРЅРіР»РёР№СЃРєРёР№":
