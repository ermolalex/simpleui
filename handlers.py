...
def java_open(hashMap,_files=None,_data=None):
    ...
    sw.setText("РљР°РєРѕР№ С‚Рѕ РїР°СЂР°РјРµС‚СЂ 1")
    
    l1 = iuClass.getView("left")
    l1.addView(sw)

    sw2 = Switch(iuClass.getContext())
    sw2.setText("РљР°РєРѕР№ С‚Рѕ РїР°СЂР°РјРµС‚СЂ 2")
...
def java_input(hashMap,_files=None,_data=None):
    ...
    sw = Switch(iuClass.getContext())
    sw.setText("РљР°РєРѕР№ С‚Рѕ РїР°СЂР°РјРµС‚СЂ РµС‰Рµ")
...
def visible_open(hashMap,_files=None,_data=None):
    ...
    #РїСѓСЃС‚РѕР№ РѕР±СЂР°Р±РѕС‚С‡РёРє, С‡С‚РѕР±С‹ РїСЂРёРјРµРЅРёР»РёСЃСЊ РїРµСЂРµРјРµРЅРЅС‹Рµ РїСЂРё РѕС‚СЂРёСЃРѕРІРєРµ  
...
def tables_open(hashMap,_files=None,_data=None):
...

 {
    ...
    for i in range(0,50):
        ...
        "key": str(i),
        "descr": "Pos. "+str(i),
        "val": str(random.randint(10, 10000))+" СЂСѓР±.",
        "pos": str(i),
        "string1": "РњР°С‚РµСЂРёРЅСЃРєР°СЏ РїР»Р°С‚Р° ASUS ROG MAXIMUS Z690 APEX",
        "string2": "Р“РЅРµР·РґРѕ РїСЂРѕС†РµСЃСЃРѕСЂР° LGA 1700",
        "string3": "Р§Р°СЃС‚РѕС‚РЅР°СЏ СЃРїРµС†РёС„РёРєР°С†РёСЏ РїР°РјСЏС‚Рё 4800 РњР“С†"
      }
        if i==2:
